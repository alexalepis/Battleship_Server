defmodule Game.Server do
  ...
  def join_player(username, client_pid, state) do
    case is_unique?(state, username) do
      true ->
        check_for_pair(username, client_pid, state)

      false ->
        Server.Protocol.send_to(client_pid, :not_unique)
  ...
  def check_for_pair(username, client_pid, state) do
    case Map.get(state, :wait_list) do
      {nil, nil} ->
        Server.Protocol.send_to(client_pid, :alone)
        IO.puts("#{username} connected successfully")
        %{state | wait_list: {username, client_pid}}

      {first_client_username, first_client_pid} ->
        {:ok, new_game_pid, state} = create_new_game(state, {username, client_pid})
        IO.puts("#{username} connected successfully")
        Server.Protocol.send_to(first_client_pid, client_pid, {:game_created, new_game_pid})

        IO.puts(
          "#{username} is playing with #{first_client_username} at the battle with ID: #{
            inspect(new_game_pid)
          }"
        )
    ...
    with new_game_id = :erlang.unique_integer(),
      ...
      message =
        Poison.encode!(%{
          ...
          "player_1" => elem(state.wait_list, 0),
          "game_id" => new_game_id
        })
