defmodule Battle do
  ...
  def next_move(game, x, y) do
    ...
    case {status, new_game, message} do
      {:error, _, :game_ended} ->
        Server.Protocol.send_to(new_game.current_player.id, new_game.enemy_player.id, message)

      {:error, _, :out_of_bounds} ->
        Server.Protocol.send_to(new_game.current_player.id, message)

      {:error, _, :already_shot} ->
        Server.Protocol.send_to(new_game.current_player.id, message)

      {:ok, _, :winner} ->
        Server.Protocol.send_to(
          new_game.current_player.id,
          new_game.enemy_player.id,
      ...
      {:ok, _, :hit} ->
        Server.Protocol.send_to(
          new_game.enemy_player.id,
          {message, new_game.enemy_player.name, new_game.enemy_player.shot_board}
        )

        Server.Protocol.send_to(
          new_game.current_player.id,
          {message, new_game.enemy_player.name, new_game.current_player.my_board}
        )
      ...
      {:ok, _, :miss} ->
        merged_map =
          Map.merge(new_game.current_player.my_board.map, new_game.enemy_player.shot_board.map)
        ...
        Server.Protocol.send_to(
          new_game.current_player.id,
          {message, new_game.enemy_player.name, merged_board}
        )
